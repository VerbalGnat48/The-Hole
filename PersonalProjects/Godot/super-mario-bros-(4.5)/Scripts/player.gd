...
const PIPE_ENTER_THRESHOLD = 5;

#On ready/Consts
const POINTS_LABEL_SCENE = preload("res://Scenes/points_label.tscn");
const SMALL_MARIO_COLLISION_SHAPE = preload("res://Collision Shapes/small_mario_collision_shape.tres");
const BIG_MARIO_COLLISION_SHAPE = preload("res://Collision Shapes/big_mario_collision_shape.tres");
...
@onready var slide_down_finished_pos = $"../slide_down_finished_position";
...
func handle_movement_collision(collision: KinematicCollision2D):
	...
	if collision.get_collider() is Pipe:
		var collision_angle = rad_to_deg(collision.get_angle() );
		if round(collision_angle) == 0 && Input.is_action_just_pressed("down") && absf(collision.get_collider().position.x - position.x < PIPE_ENTER_THRESHOLD && collision.get_collider().is_traversable):
...
func on_pole_hit():
	...
	var slide_down_position = slide_down_finished_pos.position;
	slide_down_tween.tween_property(self, "position", slide_down_position, .5);
...
func go_to_castle():
	...
	run_to_castle_tween.tween_property(self, "position", position + Vector2(40, 0), .5);
